    ...
    printResults(args, failedProjectNames, startedWithFailedProjects, cachedProjectNames) {
        ...
        if (failedProjectNames.length === 0) {
            const bodyLines = cachedProjectNames.length > 0
                ? [
                    output_1.output.colors.gray(`Nx read the output from cache instead of running the command for ${cachedProjectNames.length} out of ${this.projectNames.length} projects.`)
                ]
                : [];
