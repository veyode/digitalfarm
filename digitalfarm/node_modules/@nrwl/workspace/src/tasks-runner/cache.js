...
class CacheConfig {
    ...
    isCacheableTask(task) {
        return (this.options.cacheableOperations &&
...
class Cache {
    ...
    removeOldCacheRecords() {
        ...
        if (shouldSpawnProcess) {
            const scriptPath = path_1.join(this.root, 'node_modules', '@nrwl', 'workspace', 'src', 'tasks-runner', 'remove-old-cache-records.js');
            try {
    ...
    put(task, terminalOutputPath, folders) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            folders.forEach(f => {
                const srcDir = path_1.join(this.root, f);
                if (fs_1.existsSync(srcDir)) {
                    const cachedDir = path_1.join(td, 'outputs', f);
    ...
    copyFilesFromCache(cachedResult, outputs) {
        outputs.forEach(f => {
            const cachedDir = path_1.join(cachedResult.outputsPath, f);
            if (fs_1.existsSync(cachedDir)) {
                const srcDir = path_1.join(this.root, f);
                if (fs_1.existsSync(srcDir)) {
    ...
    createCacheDir() {
        ...
        if (this.options.cacheDirectory) {
            ...
            dir = path_1.join(this.root, 'node_modules', '.cache', 'nx');
        }
